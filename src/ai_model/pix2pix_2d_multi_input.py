...
def Generator(input_shape=(256, 256, 4)):
...
def Discriminator():
    initializer = tf.random_normal_initializer(0.0, 0.02)
    inp_1 = tf.keras.layers.Input(shape=[256, 256, 1], name='input_1')
    inp_2 = tf.keras.layers.Input(shape=[256, 256, 1], name='input_2')
    inp_3 = tf.keras.layers.Input(shape=[256, 256, 1], name='input_3')
    inp_4 = tf.keras.layers.Input(shape=[256, 256, 1], name='input_4')

    #inp = tf.keras.layers.concatenate([inp_1, inp_2, inp_3, inp_4])  # (batch_size, 256, 256, channels*4)

    tar = tf.keras.layers.Input(shape=[256, 256, 1], name='target_image')
    x = tf.keras.layers.concatenate([inp_1, inp_2, inp_3, inp_4, tar])  # (batch_size, 256, 256, channels*2)
    ...
    return tf.keras.Model(inputs=[inp_1, inp_2, inp_3, inp_4, tar], outputs=last)
...
def generate_images(model, test_input, tar, savefig=True, step=None):
    ...
    display_list = [test_input['input_1'][0, :, :].numpy(), tar[0, :, :].numpy(), prediction[0, :, :, 0].numpy()]
    title = ["Input Image", "Ground Truth", "Predicted Image"]
    for i in range(3):
        plt.subplot(1, 3, i + 1)
        plt.title(title[i])
...
def _input_fn(inputs, target):
    ...
    def generator():
      for s1, s2, s3, s4,  l in zip(sent1, sent2, sent3, sent4, target):
        yield {"input_1": s1, "input_2": s2, "input_3": s3, "input_4": s4}, l

    dataset = tf.data.Dataset.from_generator(generator, output_types=({"input_1": tf.float32,
                                                                       "input_2": tf.float32,
                                                                       "input_3": tf.float32,
                                                                       "input_4": tf.float32}, tf.float32))
...
def set_up_and_train_2d_model():
    ...
    # reshape the data
    inputs_dataset = np.reshape(inputs_dataset, (inputs_dataset.shape[1], inputs_dataset.shape[0], inputs_dataset.shape[2], inputs_dataset.shape[3] ))
...
def Generator_multi_input(input_shape=(256, 256, 1)):

    model_1 = Generator(input_shape)
    model_2 = Generator(input_shape)
    model_3 = Generator(input_shape)
    model_4 = Generator(input_shape)

    input_1 = tf.keras.Input(shape=input_shape, name="input_1")
    input_2 = tf.keras.Input(shape=input_shape, name="input_2")
    input_3 = tf.keras.Input(shape=input_shape, name="input_3")
    input_4 = tf.keras.Input(shape=input_shape, name="input_4")
...
if __name__ == "__main__":
    ...
    generator = Generator_multi_input((IMAGE_SIZE, IMAGE_SIZE, 1))
